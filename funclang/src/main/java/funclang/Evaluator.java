...
public class Evaluator implements Visitor<Value> {
	...
	public Value visit(LessExp e, Env env) { // New for funclang.
		...
		// alternate functionality for string comparison. Note that only length is considered.
		catch(Exception e1) {
			...
			return new Value.BoolVal(first.tostring().length() < second.tostring().length());
		}
	}
	...
	public Value visit(EqualExp e, Env env) { // New for funclang.
		/**
		 * Default functionality if exp is a numVal
		 */
		try {
			Value.NumVal first = (Value.NumVal) e.first_exp().accept(this, env);
			Value.NumVal second = (Value.NumVal) e.second_exp().accept(this, env);
		...
		/**
		 * If it isn't, we try to process it as a string instead.
		 */
		catch(Exception e1) {
			Value.StringVal first = (Value.StringVal) e.first_exp().accept(this, env);
			Value.StringVal second = (Value.StringVal) e.second_exp().accept(this, env);
			return new Value.BoolVal(first.tostring().equals(second.tostring()));
